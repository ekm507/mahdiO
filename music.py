...
def create_music3(notes_list, instrument, sampling_rate:float=48000, note_duration:float=0.3, note_type:str='midi', time_type='relative'): # for notes list version3
    ...
    if time_type == 'independant':
        for note, time in notes_list:
            for i in range(int(time * sampling_rate)):
                audio_wave[i + int (time * sampling_rate)] += noteFiles[note][i]
        ...
        for note, time in notes_list:
            ...
            last_time += time
            for i in range(int(note_duration * sampling_rate)):
                audio_wave[i + int(last_time * sampling_rate)] += noteFiles[note][i]
